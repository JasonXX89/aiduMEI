...
> **当前公开版本 v21.0 正式版 —— 优忆思：一行 Prompt 全自动部署 · 双引擎自动挡 · 市面独一份。**（v21.0：认知治理全量版——记忆出身标签 / 知识溯源 / 记忆档案导出；生产用户审计 2🔴3🟡3🟢 全闭环。详见 [CHANGELOG](CHANGELOG.md)。）
...
## 资源实测（口径登记在 `ducky/doc_facts.py`，改数字先改那里）

云端档常驻约 280 MB，自动/本地档约 430 MB——两档实测差 150 MB（onnxruntime 运行库 75 MB + bge-small-zh 会话与权重约 122 MB 等构成）。冷启动 5.2s；`/search` 单次 0.14~0.23s（2 核 3.5GB 生产口径，2026-08 实测）。
...
> **为什么要把 2022 和 1967 都写出来**：2022 是本树开发环境 2026-09-14 实测（缺宿主 ×12）；1967 是本树生产机独立沙箱 2026-09-11 实测（`de09794`，宿主在场但沙箱缺多项可选轴）——两者的跳过轴不同，数字必须与环境、日期和测试树一起读。
...
> ```
>
> 上面代码块里的 `有宿主：2034 passed` 要**十三条轴同时齐备**才拿得到，宿主只是其中一条 —— 别把「装上宿主」当成「全绿」。
