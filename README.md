...
| 维度 | 现状 |
|------|------|
| 用例总数 | **2101**（`pytest --collect-only` 实测，2026-09-17，v21.2-dev 本树）＝ **行为用例 1895（产品代码直测）+ 脚本/钩子行为 70 + 守卫用例 136（文档/口径/结构）**。三桶口径与名单见 `scripts/count_test_kinds.py`，可一键复算——v20.5.1 起头条不再用混合数（外部审计 C-1） |
| 独立开发机 | 2089 通过 · **12 跳过** —— **2026-09-17 实测**（v21.2-dev 本树，Python 3.12；完整 extras + 模型缓存，只缺 Hermes 宿主） |
...
> **为什么要把 2089 和 1967 都写出来**：2089 是本树开发环境 2026-09-17 实测（缺宿主 ×12）；1967 是生产机独立沙箱 2026-09-11 实测（`de09794`，宿主在场但沙箱缺多项可选轴）——两者的跳过轴不同，数字必须与环境、日期和测试树一起读（生产沙箱数待生产机 v21.1 实机复测更新）。
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：2089 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：2101 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 2089 passed, 12 skipped
> ```
>
> 上面代码块里的 `有宿主：2101 passed` 要**十三条轴同时齐备**才拿得到，宿主只是其中一条 —— 别把「装上宿主」当成「全绿」。
