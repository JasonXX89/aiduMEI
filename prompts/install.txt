...
第 9.5 步：验证写线真的在工作。先用宿主跟我**真实对话至少 5 轮**（不是脚本造的，要走你刚接的钩子），然后运行 python scripts/check_ingest_wiring.py --token "$AIDUMEM_API_TOKEN" --require-judgment。另外还有第三条「萃取线」（Hermes 挂 session_end，脚本 integrations/aidumem-distill.sh），会话结束时把这一程提炼成一条，漏挂不致命但会永远没有「这一程」那一层，装法同上、自检同上。退出码 0 才算接线成功；报「你在读但没在写」是写钩子没挂对位置，报「无法判断」是样本还不够或读线是不传 session 的旧版本 —— 三种都要回第 9 步修好再继续，不许跳过。
