...
| Dimension | Status |
|---|---|
| Total cases | **2122** (measured via `pytest --collect-only`, 2026-09-17, v21.2-dev tree) = **1916 behavior + 70 script/hook + 136 guard** (split口径 `scripts/count_test_kinds.py`) |
| Clean dev machine | 2110 passed · **12 skipped** — **measured 2026-09-17** (v21.2-dev tree, Python 3.12; complete extras and model cache, only Hermes source absent) |
...
> pytest tests/ -q -rs | tail -1                                 # no host: 2110 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # with host: 2122 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # forced off: 2110 passed, 12 skipped
> ```
>
> `2122 passed` in the block above requires **all thirteen axes present**; the host is only one of them — don't read "install the host" as "all green".
>
