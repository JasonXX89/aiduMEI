...
aiduMEI is an **AI Wisdom Engine** — a persistent memory and reasoning system for AI Agents. The current public release is **v21.0 (Preview, in-flight)** — **YouiSi: One-Line Prompt deployment, dual-engine autoshift, first of its kind.** This preview carries the EchoMind-inspired cognitive-governance batch (epistemic tagging, provenance, memory dossier, governance core behind tri-state switches with shadow-first rollout; see CHANGELOG v21). All three preview-round review findings are closed: both commitment-level defects fixed at the root, all six yellows cleared (see CHANGELOG v20.5.0). The Wisdom Engine Autoshift provides a dual engine with automatic gear shifting. When external services fail it downshifts automatically and keeps running seamlessly; when they recover it upshifts and replays the debt; the gear is always honestly visible. v20.1's "deterministic fallbacks and honest recall" (17 remediation items closed across five external reviews) is its foundation. It embodies a complete **cognitive architecture** that enables AI to **remember, think, and evolve**.
...
**Why report both 2022 and 1821**: the first is the 2026-09-14 measurement of the complete optional environment on this tree; the second is the 2026-09-09 clean-venv measurement of the basic install path (requirements files only, this tree). With the complete environment, model cache and public LoCoMo dataset present, both host states produce:

```text
no host: 2022 passed, 12 skipped
with host: 2034 passed
forced off: 2022 passed, 12 skipped
```

On a production host where other optional axes are absent, the bare command **actually prints 1967 passed, 26 skipped** (measured 2026-09-11 on the v20.5.1 tree de09794 in an isolated sandbox on the production box, no `.env`, isolated HOME). A number without its environment and date is not a reproducible claim.
