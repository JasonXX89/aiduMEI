...
- **M7 episode rollup（默认关）+ M8 借阅对齐**：同 episode 多条命中聚合为 ≤6 步轨迹摘要（拼接式零 LLM）；`grant_hall_access` / `revoke_hall_grant` 进事件账本留痕（撤销只在**真撤到了**时记账），记忆档案新增「## 八、当前生效借阅」。用例总数 2048 → 2110（+62 条验收与整改守卫，全部红→绿）；env 新键 11 个全走 `AIDUMEI_` 前缀并三处登记。施工期被本仓守卫拦下 16 次并逐条收口，其中两次是真缺陷：函数内 `import os` 遮蔽模块级绑定（P0-2 同类）、MMR 点火豁免语义写错（低分点火条会压过高分条）。
